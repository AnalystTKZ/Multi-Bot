...
== BLIND BACKTEST PIPELINE ==
  ...
  Round 3 — incremental retrain ALL models on full data
             → backtest on last 3yr (val+test overlap) for final evaluation
...
def run_retrain(model: str, label: str = "") -> None:
...
# ─── Round 3: Incremental retrain ALL models on full data ────────────────────
# Retrains GRU, Regime, Quality, RL using the complete dataset (train+val+test).
# The journal at this point contains trades from both Round 1 and Round 2.

print("\n=== Round 3: Incremental retrain ALL models on full data ===")
for model in ["gru", "regime", "quality", "rl"]:
    run_retrain(model, label="full-data retrain")
